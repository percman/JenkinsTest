...
public class EmployeeDaoImplementation implements EmployeeDao {
	...
	public List<Employee> getAllEmployees() {
		try {
			...
			PreparedStatement stmt = conn.prepareStatement("SELECT * FROM employee e FULL OUTER JOIN employee_information ei "
										+ "	ON e.employee_id = ei.employee_id ORDER BY e.employee_id");
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
	...
	public boolean insertUser(Employee employee) {
		...
		try {
			...
			System.out.println("joiajefoifjej");
	...
	@Override
	public boolean modifyUser(Employee employee) {
		int index = 0;
		try {
			System.out.println("Modifying user");
			CallableStatement stmt = conn.prepareCall("{CALL modify_employee(?,?,?,?)}");
