...
public void addMovie(String title) throws AlreadyHaveMovieException {	
	if (this.movies.contains(title))
		throw new AlreadyHaveMovieException();
	this.movies.add(title);
}
//allows a user to remove a movie from thier collection
public void removeMovie(String title) throws MovieNotFoundException, NoMovieException {	
	if(this.movies.isEmpty())
		throw new NoMovieException();
	else if (!this.movies.contains(title))
