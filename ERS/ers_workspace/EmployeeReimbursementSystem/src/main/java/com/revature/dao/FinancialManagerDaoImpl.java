...
public class FinancialManagerDaoImpl implements FinancialManagerDao {
	...
	public List<Reimbursement> viewAllReimbursements() {
		...
		try (Connection conn = ConnectionUtil.getConnection()){
			PreparedStatement stmt = conn.prepareStatement("SELECT r.reimbursement_id, (ei.f_name || ' ' || ei.l_name) AS requestor_name, s.r_status, c.r_category, r.amount, r.submitted, r.resolved, "
			...
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
				list.add(new Reimbursement(rs.getInt("r.reimbursement_id"), rs.getString("requestor_name"), rs.getString("ai.approver_name"), rs.getString("c.r_category"), rs.getString("s.r_status"), rs.getInt("r.amount"), rs.getString("r.submitted"), rs.getString("r.resolved")));
			}
			return list;
		} catch (SQLException sqle) {
			LogThis.warn(sqle.getMessage());
	...
	@Override
	public boolean resolveReimbursement(Reimbursement reimbursement, int status) {
