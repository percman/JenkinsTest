...
public class UserDAOImplementation implements UserDAO {
	...
	public boolean generateTimestamp(int id) {
		int index = 0;
		try {
			CallableStatement stmt = conn.prepareCall("{CALL insert_timestamp(?)}");
	...
	public int getBalance(int id) {
		try {
			PreparedStatement stmt = conn.prepareStatement("SELECT COUNT(user_timestamp)AS balance FROM user_timestamp WHERE accountnumber "
	...
	public int getTotalBalance() {
		try {
			PreparedStatement stmt = conn.prepareStatement("SELECT COUNT(user_timestamp)AS balance FROM user_timestamp");
			ResultSet rs = stmt.executeQuery();
			if (rs.next()) {
				return rs.getInt("balance");
			}
