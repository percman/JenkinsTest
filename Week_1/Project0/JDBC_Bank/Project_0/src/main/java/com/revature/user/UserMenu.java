	...
	public static void menu(UserInterface currentUser) {
		...
		while (choice != "1" && choice != "2" && choice != "3" && choice != "4" && choice != "9") {
			...
			case "1": { // Get account information for the user
				...
				System.out.println("Current Balance: " + UserService.getBalance(currentUser.getAccountNumber()) + " TiCos");
			...
			case "2": { // Allow the user to generate a time stamp
				...
				if(input.next().equals("1")) {
					...
					if (!UserService.generateTimestamp(currentUser.getAccountNumber())) {
						try {
			...
			case "3": { // Allow the user to make a trade
				...
				if (UserService.getBalance(currentUser.getAccountNumber()) < tradeAmount) {
					try {
				...
				UserService.makeTradeRequest(currentUser.getAccountNumber(), tradeTo, tradeAmount);
				break;
			}
			...
			case "4": { // Allow the user to accept a trade
				...
				if(accept == 1) {
					...
					UserService.acceptTradeRequest(requester, currentUser.getAccountNumber(), tradeAmount);
					break;
				}
				else if(accept == 2) {
					UserService.denyTradeRequest(requester, currentUser.getAccountNumber(), tradeAmount);
					break;
				}
				else {
					break;
