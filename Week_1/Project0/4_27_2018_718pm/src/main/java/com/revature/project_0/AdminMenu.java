			...
			case "1": { // Lock a single account
				...
				try {
					if (ListOfUsers.users.indexOf(admin) + 1 == Integer.parseInt(choice.replaceAll(" ", ""))) {
						...
						logger.trace("Administator: " + admin.getFirstName() + " " + admin.getLastName() + " locked an account.");
					}
					break;
			...
			case "2": { // Unlock a single account
				...
				try {
					if (ListOfUsers.users.indexOf(admin) + 1 == Integer.parseInt(choice.replaceAll(" ", ""))) {
						...
						logger.trace("Administator: " + admin.getFirstName() + " " + admin.getLastName() + " unlocked an account.");
					}
					break;
				...
				try {
					if (ListOfUsers.users.indexOf(admin) == 0) { // only give this access to the main admin
						if (ListOfUsers.users.indexOf(admin) + 1 == Integer.parseInt(choice.replaceAll(" ", ""))) {
							...
							logger.trace("Administator: " + admin.getFirstName() + " " + admin.getLastName() + " deleted an account.");
						}
						break;
					} else {
						System.out.println("\n\n\t\t***YOU DO NOT HAVE ACCESS TO THIS FEATURE***");
						logger.info("Administator: " + admin.getFirstName() + " " + admin.getLastName() + " denied access.");
						break;
					}
				} catch (NumberFormatException nfe) { // This will detect if a non integer is input by the user
					System.out.println("Incorrect entry detected. You must input 2 numbers seperated by a space");
				...
				try {
					if (ListOfUsers.users.indexOf(admin) == 0) { // Only give this access to the main admin
						if (ListOfUsers.users.indexOf(admin) + 1 == Integer.parseInt(choice.replaceAll(" ", ""))) {
							...
							logger.trace("Administator: " + admin.getFirstName() + " " + admin.getLastName() + " promoted an account.");
						}
						break;
					} else {
						System.out.println("\n\n\t\t***YOU DO NOT HAVE ACCESS TO THIS FEATURE***");
				...
				try {
					if (ListOfUsers.users.indexOf(admin) == 0) { // Only give this access to the main admin
						if (ListOfUsers.users.indexOf(admin) + 1 == Integer.parseInt(choice.replaceAll(" ", ""))) {
							...
							logger.trace("Administator: " + admin.getFirstName() + " " + admin.getLastName() + " demoted an account.");
						}
						break;
					} else {
						System.out.println("\n\n\t\t***YOU DO NOT HAVE ACCESS TO THIS FEATURE***");
	...
	public static void optionTwo(User admin) {
		...
		while (choice != "1" && choice != "2" && choice != "3" && choice != "4" && choice != "9") {
				...
				try {
					if (ListOfUsers.users.indexOf(admin) + 1 == Integer.parseInt(choice.replaceAll(" ", ""))) {
						...
						if (ListOfUsers.users.get(Integer.parseInt(choice.replaceAll(" ", "")) - 1).isApproved() == 2) {
							...
							logger.trace("Administator: " + admin.getFirstName() + " " + admin.getLastName() + " approved an account.");
						}
					}
					break;
			...
			case "2": { // Approve all pending accounts
				...
				logger.trace("Administator: " + admin.getFirstName() + " " + admin.getLastName() + " approved all accounts.");
				...
				try {
					if (ListOfUsers.users.indexOf(admin) + 1 == Integer.parseInt(choice.replaceAll(" ", ""))) {
						...
						if (ListOfUsers.users.get(Integer.parseInt(choice.replaceAll(" ", "")) - 1).isApproved() == 2) {
							...
							logger.trace("Administator: " + admin.getFirstName() + " " + admin.getLastName() + " denied an account.");
						}
					}
					break;
			...
			case "4": { // Deny all pending accounts
				...
				logger.trace("Administator: " + admin.getFirstName() + " " + admin.getLastName() + " denied all accounts.");
			...
			case "6": { // Delete denied accounts
				try {
					...
					logger.trace("Administator: " + admin.getFirstName() + " " + admin.getLastName() + " deleted denied accounts.");
					...
					logger.fatal("FATAL ERROR." + cme.getMessage());
			...
			case "7": { // Restore denied accounts
				...
				logger.trace("Administator: " + admin.getFirstName() + " " + admin.getLastName() + " restored denied accounts.");
