	...
	public static void menu(User user) {
		...
		while (choice != "1" && choice != "2" && choice != "3" && choice != "4" && choice != "9") {
			...
			case "2": { // Allow the user to make a deposit
				...
				try {
					String deposit = Initialization.input.next() + Initialization.input.nextLine();
	...
	public static void optionOne(User user) {
		...
		while (choice != "1" && choice != "2" && choice != "3" && choice != "4" && choice != "9") {
			...
			case "1": { // Let the user change their first name
				...
				Initialization.logger
						.trace("User: " + user.getFirstName() + " " + user.getLastName() + " first name change.");
				break;
			}
			case "2": { // Let the user change their last name
				...
				Initialization.logger
						.trace("User: " + user.getFirstName() + " " + user.getLastName() + " last name change.");
				break;
			}
			case "3": { // Let the user change their username
				...
				Initialization.logger
						.trace("User: " + user.getFirstName() + " " + user.getLastName() + " username change.");
				break;
			}
			case "4": { // Let the user change their password
				...
				Initialization.logger
						.trace("User: " + user.getFirstName() + " " + user.getLastName() + " password change.");
				break;
			}
			default: {
				System.out.println("Please enter a valid option");
